...
def build_pianoroll_figure(work: m21_stream.Stream, figsize: tuple[float, float] = (14.0, 7.0)) -> Figure:
    ...
    ax.set_xlabel("時間（拍 / 四分音符）", color=dt.MPL_LABEL, fontsize=10)
    ax.set_ylabel("音高", color=dt.MPL_LABEL, fontsize=10)
    ...
    ax.set_title("ピアノロール", color=dt.TEXT_PRIMARY, fontsize=12, fontweight="bold", pad=10)
    theme_matplotlib_figure(fig)
